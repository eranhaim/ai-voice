...
def get_pvc_captcha_audio(voice_id: str) -> bytes:
    """Fetch the captcha audio (MP3) the user must repeat for verification.

    The SDK helper discards the response body, so we hit the HTTP endpoint
    directly. The phrase changes per-voice and is randomly generated by ElevenLabs.
    """
    api_key = os.getenv("ELEVENLABS_API_KEY", "")
    if not api_key:
        raise RuntimeError("ELEVENLABS_API_KEY missing from env")
